...
server_start(){
    ...
    #echo "launchctl load -Fw $HTTPD_PLIST"
    launchctl load -Fw $HTTPD_PLIST
    sleep 1
}
server_stop(){
	echo "Shutting down Apache"
    #echo "launchctl unload $HTTPD_PLIST"
...
mysql_start(){
	echo "Starting MySQL"
    #echo "launchctl load -Fw $MYSQL_PLIST"
    launchctl load -Fw $MYSQL_PLIST
    sleep 1
}
mysql_stop(){
	echo "Shutting down MySQL"
    #echo "launchctl unload $MYSQL_PLIST"
...
postgres_start(){
    ...
    #echo "launchctl load -Fw $PGSQL_PLIST"
    launchctl load -Fw $PGSQL_PLIST
    sleep 1
}
postgres_stop(){
    echo 'Shutting down Postgresql'
    #echo "launchctl unload $PGSQL_PLIST"
